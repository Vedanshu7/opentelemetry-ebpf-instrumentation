...
// walk performs a depth-first traversal of the AST rooted at n, invoking fn
// for every node visited, including n itself. goja does not export an AST
// visitor, so the traversal is hand-rolled.
...
// children returns the direct child nodes of n that can hold route
// registrations or variable declarations; leaf nodes return nil.
...
// nodes builds a child slice from individual nodes, skipping both untyped nil
// interfaces and interfaces wrapping a nil pointer: goja uses either for
// absent optional nodes.
